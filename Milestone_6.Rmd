...
The replicable and essential figures/tables for my paper are Figures 4, 5, 6, 8, and 9, as well as Table 1. Figures 1, 2, 3, and 7 are made either via GIS software or are simply maps of Osh. These I could just screen shot and insert into my own work if necessary. There is also an extensive online Appendix, but it mostly consists of extensive robustness testing and therefore I consider it non-essential for the paper overall. Figures 4, 5, 8, 9 and Table 1 are fully replicable and are shown in the Appendix of this milestone. I cannot currently replicate all of Figure 6 from my paper.  Figure 6 has three regression series using an instrumental variable. Two of the regression series rely on the survey data they have provided, but a third regression relies on spatial weights. These spatial weights are included in a csv file, but when loaded into my rmd file, they fail to create the square matrix that the code requires. I have worked with Alice on this issue and we have not been able to reproduce their work, but I have reached out to them and hopefully they will clarify the issue. I have replicated two of the three regression series, however, which capture the overall point of this particular model and this can be seen in the Appendix as well. I also want to replicate one appendix figure, A.17 for my extension section, but it relies on the same spatial weights as Figure 6, so it is not replicable as of now.  
...
\begin{center}
\textbf{Figure A.17  Effect of Riot Destruction on Prosocial Behavior (Kyrgyz Sample)}
